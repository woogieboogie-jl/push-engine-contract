    ...
    ReentrancyGuard
{
     ...
     *
     * @param reportVersion The version of the report data. Must be between 2 and 10.
    ...
    function _updateReport(uint16 reportVersion, bytes memory verifiedReportData) internal virtual {
        ...
        if (reportVersion == 2) {
            ...
            // v5 report schema
            ReportV5 memory verifiedReport = abi.decode(verifiedReportData, (ReportV5));

            // Extract the details
            reportFeedId = verifiedReport.feedId;
            reportPrice = verifiedReport.rate;  // Use 'rate' field for V5
            reportValidFromTimestamp = verifiedReport.validFromTimestamp;
            reportTimestamp = verifiedReport.observationsTimestamp;
            reportExpiresAt = verifiedReport.expiresAt;
        } else if (reportVersion == 6) {
            // v6 report schema
            ReportV6 memory verifiedReport = abi.decode(verifiedReportData, (ReportV6));

            // Extract the details
            reportFeedId = verifiedReport.feedId;
            reportPrice = verifiedReport.price;  // Use primary 'price' field
            reportValidFromTimestamp = verifiedReport.validFromTimestamp;
            reportTimestamp = verifiedReport.observationsTimestamp;
            reportExpiresAt = verifiedReport.expiresAt;
            ...
            // Extract the details
            reportFeedId = verifiedReport.feedId;
            reportPrice = verifiedReport.exchangeRate;  // Use 'exchangeRate' field for V7
            reportValidFromTimestamp = verifiedReport.validFromTimestamp;
            reportTimestamp = verifiedReport.observationsTimestamp;
            reportExpiresAt = verifiedReport.expiresAt;
            ...
            // v9 report schema
            ReportV9 memory verifiedReport = abi.decode(verifiedReportData, (ReportV9));

            // Extract the details
            reportFeedId = verifiedReport.feedId;
            reportPrice = verifiedReport.benchmark;  // Use 'benchmark' field for V9
            reportValidFromTimestamp = verifiedReport.validFromTimestamp;
            reportTimestamp = verifiedReport.observationsTimestamp;
            reportExpiresAt = verifiedReport.expiresAt;
        } else if (reportVersion == 10) {
            // v10 report schema
            ReportV10 memory verifiedReport = abi.decode(verifiedReportData, (ReportV10));
